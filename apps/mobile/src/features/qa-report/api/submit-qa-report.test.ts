...
jest.mock('../../../../modules/qa-screen-recorder', () => ({ compressVideo: jest.fn() }));

const fetchMock = fetch as jest.MockedFunction<typeof fetch>;
const compressVideoMock = compressVideo as jest.MockedFunction<typeof compressVideo>;

it('큰 영상을 압축해 인증 없이 QA API로 보낸다', async () => {
  process.env.EXPO_PUBLIC_WEB_URL = 'https://app.example';
  compressVideoMock.mockResolvedValue({ uri: 'file:///compressed.mp4', size: 2_000_000 });
  ...
  expect(compressVideoMock).toHaveBeenCalledWith(report.video.uri, 4_000_000);
