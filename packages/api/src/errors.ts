...
/**
 * null이 정상 payload인 조회 엔드포인트용.
 * 서버가 null 필드를 직렬화에서 생략하므로 data 키가 없는 것도 null로 본다.
 */
export async function unwrapNullableData<T>(result: Promise<ApiResult<T>>): Promise<T | null> {
  const { data, error, response } = await result;
