...
- Query 훅은 `<domain>-queries.ts`, Mutation 훅은 `<domain>-mutations.ts`에 모은다.
- Query 또는 Mutation이 없는 파일은 만들지 않는다.
...
- 도메인 명령: 실제 행위의 동사인 `start`, `cancel`, `agree`, `withdraw` 등
...
- 클라이언트 인스턴스는 web에서 `@/shared/api/client`, mobile에서 `@/lib/api`를 import한다.
...
- request body와 query 타입은 `paths`에서 파생해 export한다.
- request body는 `<Action><Domain>Input`, 목록 query는 `<Domain>ListParams`를 기본으로 한다.
- 범위나 속성이 의미를 바꾸면 `<Action><Subject>Input`, `<Domain><Scope>Params`처럼 구체화한다.
- `string`으로 충분한 단일 path ID는 별도 타입을 만들지 않는다.
...
- 스키마에 query 파라미터가 있을 때만 해당 값을 Query Key에 포함한다.
...
- 인증 토큰 자동 갱신(`/auth/refresh`)은 entities API·훅으로 만들지 않고 API 클라이언트 인증 계층에서 처리한다.
